...
const PER_PLAYER = {
  ...
  winter: 6000,
  spring: 6000,
  bundle: 18000,  // all three tours, paid at Fall registration
  // Live-mode smoke test: a real charge small enough not to matter, so the
  // whole path can be proven with real money before players arrive. Hidden on
  // the signup page unless ?test=1 is in the URL.
  test: 30
};
...
const TOUR_LABELS = {
  ...
  test: "Test entry (not a real registration)"
};

const PLATFORM_LABELS = {
  sgt: "SGT / GSPro",
  trackman: "Trackman",
  // Used when a club's technology is not on file yet. The player is never
  // asked to guess, and the series is settled from the admin page before the
  // tour starts rather than putting the buy-in in the wrong pot.
